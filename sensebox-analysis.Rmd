...
The openSenseMap currently provides access to `r nrow(all_boxes)` senseBoxes of which `r nrow(pm25_sensors)` provide measurements of [PM2.5](https://www.umweltbundesamt.de/sites/default/files/medien/377/dokumente/infoblatt_feinstaub_pm2_5_en.pdf) around `r format(analysis_date, "%Y-%m-%d %T %Z")`.
...
pm25_sensors_sf <- st_as_sf(pm25_sensors, remove = FALSE, agr = "identity")
names(pm25_sensors_sf) <- c(names(pm25_sensors), "geometry")

pm25_sensors_sf <- cbind(pm25_sensors_sf, dist_to_ms = st_distance(ms, pm25_sensors_sf))
max_dist <- set_units(3, km) # km from city center

ms_sensors <- pm25_sensors_sf[pm25_sensors_sf$dist_to_ms < max_dist,c("X_id", "name")]
ms_sensors
```

Now we retrieve data for `r nrow(ms_sensors)` senseBoxes with values in the area of interest.

```{r muenster_data, results='hide'}
...
top_station_coords <- ms_data %>% filter(sensorId == top_sensor) %>% select("lat", "lon") %>% slice(c(1))
top_station <- all_boxes %>% filter(lon == top_station_coords$lon, lat == top_station_coords$lat)
```

Congratulations (?) to sensor _`r top_sensor`_ at box **`r top_station$name`** for holding the record values just after the new year started.
Where is this box?
